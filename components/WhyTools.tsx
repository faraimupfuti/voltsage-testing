...
const WHY_META = [
  { icon: Calculator, color: '#2621FF' },
  { icon: ShieldCheck, color: '#0B1220' },
  { icon: BadgeCheck, color: '#1A2030' },
  { icon: Clock, color: '#171254' },
  { icon: MapPin, color: '#4C46FF' },
  { icon: Gift, color: '#64748b' },
]
const TOOLS_META = [
  { href: '#sizing', color: '#0B1220', directUrl: toolUrl('residential') },
  { href: '#agricultural', color: '#1A2030', directUrl: toolUrl('agricultural') },
  { href: '#battery', color: '#2621FF', directUrl: toolUrl('battery') },
  { href: '#dc-cable-sizing', color: '#171254', directUrl: toolUrl('dcCable') },
]
      ...
      <section id="why" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center max-w-3xl mx-auto mb-6">
            ...
            <h2 className="font-disp font-extrabold text-4xl sm:text-5xl text-ink leading-tight mb-5">{t.why1.h2a}<br/><span className="brand-text">{t.why1.h2b}</span></h2>
            ...
            {WHY.map((b,i)=>(
              <Reveal key={i} delay={i*70} className="card p-6 group relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="font-disp font-bold text-sm text-ink leading-tight">{b.title}</h3>
                </div>
      ...
      <section className="py-24 bg-subtle">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="max-w-2xl mb-12">
            ...
            <h2 className="font-disp font-extrabold text-4xl sm:text-5xl text-ink leading-tight mb-4">{t.why2.h2a}<br/><span className="brand-text">{t.why2.h2b}</span></h2>
            ...
            {TOOLS.map((t2,i)=>(
              <Reveal key={i} delay={i*90} className="gradient-border rounded-2xl overflow-hidden">
                <div className="card-flat p-6 h-full flex flex-col">
                  ...
                  <h3 className="font-disp font-bold text-xl text-ink mb-3">{t2.name}</h3>
          ...
          <div className="mt-12 rounded-2xl border border-surface-border bg-white p-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
              <div>
                <h3 className="font-disp font-bold text-2xl text-ink mb-3">{t.why2.compareTitle}<span className="brand-text-teal">{t.why2.compareTitleAccent}</span>{t.why2.compareTitlePost}</h3>
                ...
                {t.why2.points.map((p,i)=>{
                  ...
                  return (
                    <div key={i} className="flex items-start gap-3">
                      <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 text-[10px] font-bold ${ok?'text-white':'text-red-500 bg-red-50 border border-red-200'}`} style={ok?{background:'linear-gradient(135deg,#1A2030,#0B1220)'}:{}}>
                        {ok?'✓':'✕'}
                      </span>
