...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled?'bg-white/95 backdrop-blur-md shadow-sm border-b border-surface-border':'bg-white/80 backdrop-blur-sm'}`}>
      <div className="h-0.5 w-full" style={{background:'linear-gradient(90deg,#2621FF,#4C46FF,#C6741E,#0B1220)'}}/>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-5">
          {NAV.map(n=><a key={n.href} href={n.href} className="text-[11px] font-mono uppercase tracking-widest text-ink-muted hover:text-brand-orange transition-colors">{n.label}</a>)}
        </nav>
      ...
      <div className={`lg:hidden bg-white border-b border-surface-border overflow-hidden transition-all duration-300 ${open?'max-h-96 opacity-100':'max-h-0 opacity-0'}`}>
        <div className="px-4 py-4 flex flex-col gap-1">
          {NAV.map(n=><a key={n.href} href={n.href} onClick={()=>setOpen(false)} className="py-3 px-3 text-sm font-mono uppercase tracking-wider text-ink-muted hover:text-brand-orange hover:bg-surface-subtle rounded-xl transition-all">{n.label}</a>)}
